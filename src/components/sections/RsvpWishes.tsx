  ...
  return (
    <SectionWrapper withOrnament variant="alternate" className="py-20 md:py-32 text-center max-w-6xl mx-auto w-full relative z-10">
      <motion.div variants={fadeUpVariant} className="flex flex-col items-center justify-center mb-12 md:mb-20 relative z-10 px-4">
        ...
        <p className="max-w-xl mx-auto text-navy-800/60 mt-4 text-sm font-body italic">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-8 lg:gap-16 text-left px-6 relative z-10 max-w-5xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input 
                  ...
                  className="w-full px-5 py-3 rounded-xl border border-navy-800/20 bg-navy-800/5 text-navy-950/70 focus:outline-none transition-all font-body text-sm cursor-not-allowed"
                />
              </div>
              
              <div>
                <label htmlFor="attendance" className="block text-xs font-body uppercase tracking-[0.2em] text-navy-800/80 font-bold mb-2 ml-1">Konfirmasi Kehadiran</label>
                <select 
                  id="attendance" 
                  name="attendance" 
                  required 
                  className="w-full px-5 py-3 rounded-xl border border-navy-800/20 bg-white/50 text-navy-900 focus:ring-1 focus:ring-navy-800 focus:border-navy-800 outline-none transition-all font-body text-sm"
                >
              ...
              <div>
                <label htmlFor="message" className="block text-xs font-body uppercase tracking-[0.2em] text-navy-800/80 font-bold mb-2 ml-1">Kirim Doa & Harapan</label>
                <textarea 
                  id="message" 
                  name="message" 
                  required 
                  rows={4}
                  className="w-full px-5 py-3 rounded-xl border border-navy-800/20 bg-white/50 text-navy-900 focus:ring-1 focus:ring-navy-800 focus:border-navy-800 outline-none transition-all placeholder:text-navy-900/30 resize-none font-body text-sm leading-relaxed"
                  placeholder="Tuliskan harapan baik Anda..."
                />
              </div>

              {status && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className={`p-4 rounded-xl text-xs font-body tracking-wider items-center flex justify-center gap-3 animate-fade-in ${status.type === 'success' ? 'bg-slate-100 text-navy-800 border border-navy-800/20' : 'bg-red-50 text-red-800 border border-red-200'}`}>
                  {status.text}
                </motion.div>
              )}
        ...
        <motion.div variants={fadeUpVariant} className="h-full flex-1 min-h-[400px]">
          ...
          <div className="flex items-center justify-between mb-8 pb-5 border-b border-navy-800/10">
             ...
             <span className="text-navy-800/40 font-body text-[10px] uppercase tracking-[0.2em] font-bold">
          ...
          <div className="flex-1 overflow-y-auto max-h-[350px] pr-2 space-y-2 custom-scrollbar scrollbar-thin scrollbar-thumb-gold-400/30 scrollbar-track-transparent">
            <AnimatePresence>
              {messages.length === 0 ? (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center h-48 text-navy-900/30 font-body text-sm italic">
                  ...
                  >
                    <div className="flex items-center justify-between gap-3 mb-1.5">
                      <div className="flex items-center gap-2">
                        <div className="h-6 w-6 bg-navy-950 text-gold-400 rounded-md flex items-center justify-center font-heading font-black text-[10px] shadow-sm">
                          {msg.name.charAt(0).toUpperCase()}
                        </div>
                        <p className="font-heading text-[13px] text-navy-950 font-bold tracking-tight truncate max-w-[150px]">{msg.name}</p>
                      </div>
                    ...
                    <p className="text-navy-950/70 text-[11px] leading-relaxed whitespace-pre-line font-body italic pl-2 border-l border-gold-400/20">
                      &quot;{msg.message}&quot;
                    </p>
                  </motion.div>
                ))
