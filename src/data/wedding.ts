export const weddingData = {
  couple: {
    groom: {
      name: "Aep Saepudin",
      shortName: "Aep",
      parents: "Putra dari Bapak Suhala & Ibu Esih",
      instagram: "@aepaef",
      phone: "6282214074959",
    },
    bride: {
      name: "Ai Rosita",
      shortName: "Ai",
      parents: "Putri dari Bapak Banan & Ibu Kuswati",
      instagram: "@airositabentar",
      phone: "6282315555213",
    },
  },
  events: {
    akad: {
      name: "Akad Nikah",
      date: "2026-04-18",
      time: "09.00",
      location: "Pagelaran, Cianjur",
      address: "Selagedang, Pagelaran, Cianjur Regency, West Java",
      mapUrl: "https://maps.app.goo.gl/m3gaZ5KQJuz1CnXy6",
    },
    resepsi: {
      name: "Resepsi Pernikahan",
      date: "2026-04-18",
      time: "12:00 WIB - Selesai",
      location: "Pagelaran, Cianjur",
      address: "Selagedang, Pagelaran, Cianjur Regency, West Java",
      mapUrl: "https://maps.app.goo.gl/m3gaZ5KQJuz1CnXy6",
    },
  },
  banks: [
    {
      name: "BCA",
      account: "2831793327",
      owner: "Aep Saepudin",
    },
    {
      name: "Mandiri",
      account: "1320030730908",
      owner: "Ai Rosita",
    }
  ],
  quranVerse: {
    arabic: "وَمِنْ ءَايَٰتِهِۦٓ أَنْ خَلَقَ لَكُم مِّنْ أَنفُسِكُمْ أَزْوَٰجًا لِّتَسْكُنُوٓا۟ إِلَيْهَا وَجَعَلَ بَيْنَكُم مَّوَدَّةً وَرَحْمَةً",
    translation: "Dan di antara tanda-tanda kekuasaan-Nya ialah Dia menciptakan untukmu isteri-isteri dari jenismu sendiri, supaya kamu cenderung dan merasa tenteram kepadanya, dan dijadikan-Nya diantaramu rasa kasih dan sayang.",
    surah: "QS. Ar-Rum: 21",
  },
};
